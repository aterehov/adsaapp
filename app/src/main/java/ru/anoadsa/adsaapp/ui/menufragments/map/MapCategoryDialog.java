...
import ru.anoadsa.adsaapp.databinding.DialogMapCategoryBinding;
...
public class MapCategoryDialog extends DialogFragment {
    private DialogMapCategoryBinding binding;
...
//    private Button fillGeoButton;
    ...
    private TitleSpinnerView categoryTSV;
...
//    }
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        binding = DialogMapCategoryBinding.inflate(getActivity().getLayoutInflater(), null, false);
...
//        fillGeoButton = binding.incidentFillGeolocationButton;
        ...
        categoryTSV = binding.incidentOrganisationCategoryTSV;
...
//        addrTTI.setInputText("Подождите...");
        ...
        categoryTSV.setEnabledTSV(false);

        viewModel.getCategorySprav().observe(this, new Observer<IncidentAbonentCategorySprav>() {
            @Override
            public void onChanged(IncidentAbonentCategorySprav sprav) {
                if (sprav == null) {
                    viewModel.loadCategorySprav();
                    return;
                }
                categoryTSV.setItems(sprav.getNames());
                if (viewModel.getSelectedCategoryName().getValue() != null) {
                    categoryTSV.setSelectedItem(viewModel.getSelectedCategoryName().getValue());
                }
                categoryTSV.setEnabledTSV(true);
            }
        });
...
//                    }
//                    viewModel.setCategoryDialogFinished(true);
                    if (categoryTSV.getSelection() != null) {
                        viewModel.setSelectedCategoryName(categoryTSV.getSelection());
                    }
                    dialog.cancel();
                }
            });
